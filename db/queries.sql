...
-- утянула бы средний рейтинг города вверх.
-- Фильтр rating IS NOT NULL на само среднее не влияет - avg() в postgres
-- пропускает NULL, а не считает его нулём. Он нужен для столбца companies:
-- без него в нём оказались бы и компании без оценки, и число перестало бы
-- отвечать на вопрос «сколько оценок легло в это среднее».
SELECT
  city,
