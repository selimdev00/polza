...
- **Веб-страница** - `src/app/companies` (список, поиск, фильтр по городу, пагинация).
...
Ссылка на задеплоенную версию добавляется сюда отдельным коммитом после деплоя (ещё не сделан на момент написания этого README).
...
Второй проход - это то, что поймало шесть "теневых" дублей в `review.csv` (`c_900006`...`c_900011`): те же компании, что уже загружены из API, но под новым пространством id и без кавычек в названии. По `ext_id` они прошли бы как новые записи. Реальный вывод загрузчика (`.superpowers/sdd/.../task-9-report.md`, `npm run load:review`):
...
Честно про текущее состояние: при 1184 строках эффект от этой формы `WHERE` уже виден, но не так, как я ожидал заранее. Запустил `EXPLAIN ANALYZE` без каких-либо настроек:
...
И с `SET enable_seqscan = off` - план не поменялся, потому что планировщик и без этой настройки уже выбрал индекс:

```
docker exec polza-db psql -U polza -d polza -c "SET enable_seqscan = off; EXPLAIN ANALYZE SELECT id FROM companies WHERE name ILIKE '%Сфера%';"

 Bitmap Heap Scan on companies  (cost=17.52..59.87 rows=24 width=8) (actual time=0.029..0.095 rows=30 loops=1)
   Recheck Cond: (name ~~* '%Сфера%'::text)
   Heap Blocks: exact=22
   ->  Bitmap Index Scan on companies_name_trgm_idx  (cost=0.00..17.51 rows=24 width=0) (actual time=0.021..0.022 rows=30 loops=1)
         Index Cond: (name ~~* '%Сфера%'::text)
 Planning Time: 0.851 ms
 Execution Time: 0.161 ms
```

Чтобы увидеть настоящий seq scan для сравнения, пришлось явно выключить оба индексных пути (`enable_indexscan` и `enable_bitmapscan`):

```
 Seq Scan on companies  (cost=0.00..64.80 rows=24 width=8) (actual time=0.005..0.588 rows=30 loops=1)
   Filter: (name ~~* '%Сфера%'::text)
   Rows Removed by Filter: 1154
 Planning Time: 0.437 ms
 Execution Time: 0.613 ms
```

Оценка стоимости у планировщика для двух путей на этом объёме близкая (59.87 у индекса против 64.80 у seq scan) - индекс уже выигрывает, но с небольшим перевесом, а не с разгромным. Таблица `companies` сейчас занимает 50 страниц на 1184 строки (`pg_class.relpages`), это не тот "девять страниц - планировщик по любому берёт seq scan" случай, который я предполагал до того, как реально это проверил. По мере роста таблицы разрыв в стоимости будет только увеличиваться в пользу индекса, а форма запроса (сравнение колонки с константой, а не `OR`-заглушка) - это то, что вообще оставляет планировщику выбор. Пишу как есть, а не как думал заранее: индекс уже используется на этом объёме, и это можно проверить одной командой `EXPLAIN`.
...
Проверял страницу `/companies` вручную в Chrome (`npm run dev`, порт 3000), на реальной загруженной базе из 1184 компаний, без запуска миграций и загрузчиков заново. Прогнал список, поиск, фильтр по городу, комбинацию поиска и города, сброс страницы при смене фильтра, прямой заход по расшаренной ссылке с `page=2`, выход за границу пагинации, пустую выдачу и консоль/сеть на утечку строки подключения к базе - весь список с результатами в `.planning/verification-log.md`. В процессе нашёл и исправил три реальных бага.
...
47 тестов в трёх файлах: `tests/csv.test.ts` (8 тестов на парсер CSV - поля в кавычках с запятыми и переносами строк, экранированные кавычки, CRLF, пустые строки), `tests/normalize.test.ts` (26 тестов на восстановление моджибейка, нормализацию города через точное совпадение/латинские алиасы/расстояние Левенштейна, разбор рейтинга, числа отзывов, сайта и телефона), `tests/dedup.test.ts` (13 тестов на ключ дедупликации и распознавание сдвига колонок).
