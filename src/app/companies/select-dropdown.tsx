...
// Общий ARIA-listbox для всех выпадающих одиночных выборов на странице -
// город, категория, размер страницы. Раньше город и категория были двумя
// почти идентичными файлами по 280 строк каждый; здесь клавиатурная
// навигация, ARIA-разметка, фокус и три визуальных состояния (наведение,
// клавиатурно-активный пункт, выбранный пункт) написаны один раз, и три
// использования расходиться между собой уже не могут - можно исправить
// баг или поменять поведение в одном месте.
...
// Значения в любом из трёх списков различаются уже по первой букве/цифре,
// копить строку незачем.
...
export function SelectDropdown({
  ...
  ariaLabel: string;
  // Город и категория делят одну ширину триггера (на телефоне - во всю
  // ширину строки, от sm - фиксированные 14rem), у размера страницы список
  // короче ("25"/"50"/"100") и не должен растягиваться на ту же ширину
  // впустую ни на каком экране - поэтому ширина параметризована целиком (не
  // только "хвост" после sm:), чтобы значение по умолчанию (w-full) не
  // конфликтовало с переопределением у вызова с других брейкпоинтов.
  ...
  const { mounted: showList, onAnimationEnd } = useClosingTransition(open);
  ...
  function selectOption(option: SelectOption): void {
    // onChange уходит немедленно, синхронно, до closeList() - применение
    // фильтра (и его переход по URL) не должно ждать анимацию закрытия
    // списка, она чисто косметическая.
  ...
  function onListKeyDown(event: React.KeyboardEvent<HTMLUListElement>): void {
    switch (event.key) {
      ...
      case ' ':
        ...
        break;
      case 'Escape':
        // Escape закрывает сразу и предсказуемо: то же setOpen(false), что и
        // везде - никакого отдельного "мгновенного" пути в обход анимации не
        // нужно, потому что сам факт закрытия (фокус, доступность варианта
        // выбора) не ждёт её конца, ждёт её только исчезновение из DOM.
        event.preventDefault();
        closeList();
        triggerRef.current?.focus();
        break;
  ...
  return (
    <div ref={containerRef} className={`relative text-sm ${widthClassName}`}>
      <button
        ref={triggerRef}
        type="button"
        role="combobox"
